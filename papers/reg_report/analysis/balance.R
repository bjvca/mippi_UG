...
baseline_dta$log_yield <-  log(baseline_dta$yield)

bal_vars <- c("age","prim_ed","HH_male", "hh_size", "dist_ag", "quality_use_any","baz_rand","source_rand", "recycler", "log_yield" )
...
for (i in 1:length(bal_vars)){
  ...
  vcov_cluster <- vcovCR(ols,cluster=baseline_dta$clusters,type="CR3")
...
save(df_balance,file=paste(path,"/papers/reg_report/results/df_balance.Rdata",sep="/"))
save(df_balance_pool,file=paste(path,"/papers/reg_report/results/df_balance_pool.Rdata",sep="/"))
save(df_means,file=paste(path,"/papers/reg_report/results/df_means.Rdata",sep="/"))
